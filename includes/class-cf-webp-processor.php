...
class Cf_Webp_Processor {
	...
	public function process_attachment( $metadata, $attachment_id ) {
		...
		if ( ! in_array( $mime_type, array( 'image/jpeg', 'image/png' ) ) ) {
			error_log( 'CF WebP: Skipping - not JPG or PNG' );
			return $metadata;
		}
	...
	$this->process_image_sizes( $attachment_id, $metadata, $file_path );
		...
		if ( isset( $this->options['enable_r2'] ) && $this->options['enable_r2'] ) {
            ...
            if ( ! is_wp_error( $upload_result ) ) {
                ...
                // $this->r2_client->upload_file( $file_path, str_replace( $upload_dir['basedir'] . '/', '', $file_path ), $mime_type );
...
public function replace_content_urls( $content ) {
    ...
    foreach ( $matches[1] as $img_url ) {
        ...
        if ( strpos( $img_url, '.webp' ) !== false || strpos( $img_url, $upload_url ) === false ) {
            continue;
        }
        ...
        if ( ! $attachment_id ) {
            ...
            $base_url = preg_replace( '/-\d+x\d+(?=\.(jpe?g|png))/i', '', $img_url );
            $base_url = preg_replace( '/-scaled(?=\.(jpe?g|png))/i', '', $base_url );
